...
library(RTextTools) # for tdm error
setwd("E:\\Study\\Machine Learning\\R\\DATA\\Sample\\enron1")
...
train.test <- function(training=0.9){
  ...
  spamTrainingCorpus<-sample(spam.training,N,replace = T)
  ## Create TermDocumentMatrix for spam training
  spam.TDM <- TermDocumentMatrix(spamTrainingCorpus, control=(list(wordLengths=c(1,Inf))))
 
  cat("Spam working dir: ---------------------------------------------------")
  ...
  hamTrainingCorpus<-sample(ham.training,N,replace = T)
  ham.TDM <- TermDocumentMatrix(hamTrainingCorpus, control=(list(wordLengths=c(1,Inf))))
  cat("Ham working dir: ---------------------------------------------------")
  ## Create TermDocumentMatrix for test
  finalTestingCorpus<-sample(c(spam.test,ham.test),N,replace = T)
  test.TDM <- TermDocumentMatrix(finalTestingCorpus, control=(list(wordLengths=c(1,Inf))))
  ...
  cat("R.data file formation done ---------------------------------------------------")
}
